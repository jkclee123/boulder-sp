...
import '../css/SellMarketPassModal.css';
...
interface SellMarketPassModalProps {
  isOpen: boolean;
  onClose: () => void;
  pass: any;
...
const SellMarketPassModal: React.FC<SellMarketPassModalProps> = ({ isOpen, onClose, pass, onTransferSuccess }) => {
  const { user } = useAuth();
    ...
    try {
      ...
      if (foundUser) {
        ...
        if (foundUser.id === user?.uid) {
          alert('You cannot sell passes to yourself.');
          return;
        }
  ...
  const executeTransfer = async () => {
    ...
    if (countValue > pass.count) {
      alert('Sale count cannot exceed available passes.');
      return;
    }
    ...
    try {
      ...
      const transferFunction = httpsCallable(functions, 'sellMarketPass');
      ...
      alert('Market pass sale successful!');
      onTransferSuccess();
      onClose();
      resetModal();
    } catch (error: any) {
      console.error('Error executing market pass sale:', error);
      const errorMessage = error.message || 'Error executing sale. Please try again.';
        ...
        <div className="modal-body">
          {step === 'search' && (
            <div className="search-step">
              <div className="pass-info">
                ...
                <p>Available: {pass.count} punch(es)</p>
          ...
          {step === 'details' && recipient && (
            <div className="details-step">
              <h3>Market Pass Sale Details</h3>

              <div className="transfer-form">
                <div className="form-group">
                  <label>Number of market passes to sell:</label>
                  <input
                    type="text"
                    inputMode="numeric"
                ...
                <div className="transfer-summary">
                  ...
                  <p><strong>Market Passes:</strong> {parseInt(formData.count.toString()) || 0}</p>
                  <p><strong>Price per pass:</strong> ${(parseFloat(pass.price.toString()) || 0).toFixed(0)}</p>
                  <p><strong>Total:</strong> ${((parseInt(formData.count.toString()) || 0) * (parseFloat(pass.price.toString()) || 0)).toFixed(2)}</p>
              ...
              <div className="action-buttons">
                ...
                >
                  {loading ? 'Selling...' : 'Confirm Market Pass Sale'}
                </button>
              </div>
            </div>
          )}
...
export default SellMarketPassModal;
